...
<head>
	...
	<title>S-PS - Kvinesdal og Flekkefjord</title>
...
<body>
  ...
  include 'array-variabler.php';
    ...
    $sql = "SELECT status, sortering, count(status) as antall
            ...
            from Requests
            where RestrictedTeamId IN(30, 29)
		...
		$sql_tid_uke1 = "SELECT datediff (minute, r.Created, t.Resolved) as tid
   							...
   							and r.RestrictedTeamId in (30, 29)
				...
				<div class="row">
			        ...
			        while( $row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC) ) {
								...
								if ($row['status'] == "NYE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['FVG']." OG OPPRETTET ".$valgt_periode."";
								}elseif ($row['status'] == "ÅPNE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['FVG']."";
								}elseif ($row['status'] == "LØSTE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['FVG']." OG LØST ".$valgt_periode."";
								}else {
