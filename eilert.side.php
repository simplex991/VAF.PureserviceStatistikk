...
<body>
  ...
  include 'array-variabler.php';
    ...
    $sql = "SELECT status, sortering, count(status) as antall
            ...
            from Requests
            where RestrictedTeamId IN(28, 35, 36)
		...
		$sql_tid_uke1 = "SELECT datediff (minute, r.Created, t.Resolved) as tid
   							...
   							and r.RestrictedTeamId in (28, 35, 36)
				...
				<div class="row">
			        ...
			        while( $row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC) ) {
								...
								if ($row['status'] == "NYE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['Eilert']." OG OPPRETTET ".$valgt_periode."";
								}elseif ($row['status'] == "ÅPNE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['Eilert']."";
								}elseif ($row['status'] == "LØSTE SAKER") {
									$saker_undertekst = "TILDELT ".$avdelinger['Eilert']." OG LØST ".$valgt_periode."";
								}else {
